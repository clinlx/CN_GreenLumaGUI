...
О: Это нормальное рабочее состояние программы:

Люди, часто пользующиеся семейным доступом или возвращавшие игры, могли заметить: если игра уже загружена, но вы потеряли право на её запуск (например, оформили возврат или кто-то из семьи сейчас в неё играет), Steam не показывает кнопку «Играть» — вместо неё появляется кнопка «Купить».

Истинная функция этой программы — принудительно превратить эту кнопку «Купить» обратно в кнопку «Играть», позволяя вам всё равно запустить игру.

Если у вас вообще нет игры и на компьютере нет её загруженных файлов, то не видеть её в библиотеке совершенно нормально — нет и кнопки, которую можно было бы восстановить, а значит, функция программы просто не может проявиться.
...
Кроме того, когда программа активна, функции Steam, связанные с загрузкой, в настоящее время не работают. Для игр, которыми вы владеете, перезапустите Steam обычным способом перед загрузкой или обновлением.
...
#### В: Для чего нужна страница «Пул приложений для подмены»?

О: Разблокировка работает по принципу «временно отказаться от владения одной игрой, чтобы получить владение другой» — владение приложением из пула временно передаётся игре, которую вы хотите разблокировать. Поэтому сколько в пуле доступных приложений — столько игр максимум можно разблокировать одновременно: **количество доступных и есть лимит разблокировки** (число в правом нижнем углу окна).

На этой странице вы управляете пулом:

* По умолчанию в пуле набор встроенных бесплатных приложений (серые, их нельзя удалить, но можно отключить). По умолчанию они скрыты — отметьте «Показать встроенный пул», чтобы увидеть их.
* Чтобы поднять лимит, введите AppID игры, которой вы уже владеете (именно на своём аккаунте — игры других участников семейной группы не подходят), но пока не собираетесь играть. Каждая добавленная запись повышает лимит на 1.
* У каждой записи есть флажок; снятие флажка временно отключает её, отключённые записи не учитываются в количестве доступных.
* Выпадающий список фильтра переключает «Показать все / Только включённые / Только отключённые».
* Кнопка «Показать соответствия» открывает окно со списком реальных пар «AppID из пула → название игры(ID игры)» в текущей конфигурации. Строки с отступом — DLC.

Внимание: пока игра из пула реально используется для подмены, вы не сможете нормально играть в неё во время сеанса с разблокировкой. Чтобы снова играть, просто отключите или удалите эту запись — на ваше настоящее владение игрой это никак не влияет.

Всё вышесказанное обусловлено тем, как устроен сам GreenLuma. Все данные пула являются сугубо локальными и никак не взаимодействуют с облаком.


#### В: Из множества игр не разблокировались лишь несколько. Что делать?

О: Скорее всего, одно или два приложения во встроенном пуле AppID стали недействительными (например, то бесплатное приложение убрали из магазина или оно больше не доступно всем). С какой игрой окажется в паре недействительное приложение из пула — та игра и не разблокируется.

**Быстрое решение**: снимите отметку с одной-двух игр в начале списка. Тогда весь порядок разблокировки сдвинется, и игра, которой досталось недействительное приложение, получит работающее. Для начала можно играть так.

**Более точное решение**: если вы знаете, какая именно игра сбоит, нажмите «Показать соответствия» на странице «Пул приложений для подмены», найдите AppID из пула в паре с этой игрой и вручную **отключите** эту запись в списке. После отключения она больше не участвует в подмене, а все последующие записи сдвинутся автоматически.

(Кстати, именно поэтому «разблокировалась только часть DLC» иногда выглядит как обрыв на определённой строке — причина та же.)


#### В: Почему на количество разблокировок есть лимит?

О: GreenLuma нужно подменить ID вашей игры, чтобы обойти проверку Steam, а бесплатных игр (доступных всем), которые можно использовать для замены, ограниченное количество — поэтому некоторый лимит существует. Число в правом нижнем углу программы и есть текущий фактический лимит: он равен количеству доступных записей в «Пуле приложений для подмены», где вы можете сами добавлять, удалять и отключать записи.
...
[2] Замените DLL-файл: Поместите ваш файл GreenLuma_2026_x**.dll в папку "override", не переименовывая его.
