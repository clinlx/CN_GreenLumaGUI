...
經常玩家庭共享，或者退款過遊戲的人可能注意到：如果已經下載了一個遊戲，而且你失去了遊玩權(可能是你退款了，或者家庭的其他人在玩)的情況下，Steam不會給你提供「開始遊戲」的按鈕，取而代之的是一個「購買」按鈕。

這個軟體的真正功能是，將這個「購買」按鈕強制變回「開始遊戲」按鈕，使得你可以強行啟動遊戲。

如果你根本沒有遊戲，本機也沒有下載好的遊戲檔案，那麼在庫裡面看不到遊戲也是正常的，自然也就看不到這個按鈕，同時這個軟體的功能就完全無法體現。

如果你覺得這款軟體可以無條件讓你下載你沒有的遊戲，那可能你對這款軟體的功能存在誤解。

另外，在功能生效時，Steam裡面和下載有關的功能目前不能工作。對於你自己的遊戲，請以正常方式啟動Steam後再下載更新。
...
#### 問: 「用於替換的app池」這個頁面是做什麼的？

答：解鎖的原理是「暫時放棄一個遊戲的所有權，來兌換另一個遊戲的所有權」——把池中某個app的所有權，暫時讓給你想解鎖的遊戲。所以池中有多少個可用的app，就最多能同時解鎖多少個遊戲，**可用數量就是解鎖上限**（介面右下角顯示的那個數字）。

這個頁面就是管理這個池子的：

* 池裡預設是軟體內建的一批免費app（顯示為灰色，不可刪除，但可以停用）。預設它們是隱藏的，勾選「顯示內建池」才會列出來。
* 想提高上限，就輸入你已經擁有（指你自己帳號擁有，不包括家庭群組其他人擁有的）、但暫時不想遊玩的遊戲的AppID。每加一個，上限就+1。
* 每一項前面有勾選框，取消勾選即為「暫時停用」，被停用的項不計入可用數量。
* 可以用篩選下拉框切換「全部顯示 / 僅顯示未停用的 / 僅顯示已停用的」。
* 點「檢視對應關係」按鈕，會彈出視窗列出目前設定裡「池APPID → 遊戲名(遊戲ID)」的實際對應關係，縮排的行是DLC。

注意：被放進池子並實際用於替換的遊戲，在解鎖啟動期間你將無法正常遊玩它。想玩回來時，停用或刪除該項即可，不影響你對它的真實所有權。

以上機制皆源於 GreenLuma 本身的實現方式，所有替換池資料均為純本機數據，不會以任何形式與雲端互動。


#### 問: 一堆遊戲裡面只有個別幾個沒有解鎖成功，怎麼辦？

答：很可能是內建的APPID池子裡有一兩個app已經失效了（例如那個免費app被下架、或不再是所有人都能存取）。失效的池app替換到哪個遊戲，那個遊戲就解鎖不了。

**臨時的辦法**：先取消前面一兩個遊戲的勾選，這樣後面遊戲的解鎖順序就會整體變化，原本對上失效app的遊戲就會換到一個正常的app上，你可以先這樣湊合玩著。

**更準的辦法**：如果你知道是第幾個遊戲出問題，可以點「用於替換的app池」頁面裡的「檢視對應關係」按鈕，找到那個遊戲對應的池APPID，然後在清單裡手動把這一項**停用**掉。停用後它就不再參與替換，後面的項會自動順延。

（順帶一提，這也是為什麼「DLC只解鎖了一部分」有時會表現得像在某一行截斷——道理是一樣的。）
...
答：因為 GreenLuma 需要將你的遊戲 ID 偽裝成其他遊戲 ID 來繞過 Steam 的驗證，而能用來替換的免費遊戲（所有人都有權存取的）數量有限，所以存在某個上限。軟體界面右下角顯示的就是目前實際的上限，它等於「用於替換的app池」中可用的數量，你可以在該頁面自行新增、刪除和停用。
...
[2] 替換 dll 文件: 將你從官方論壇解壓出來的 GreenLuma_2026_x**.dll 文件放入 override 資料夾下，不要改名
