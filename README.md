...
## 我也看到了新版本Steam封号热点传言。
## 不过根据我的理解：这个逆向的结果是Steam弹窗提醒玩家。弹窗文本是: "xxx"目录下的"xxx"软件会导致Steam奔溃，并且“可能”导致VAC，请你卸载后再运行Steam。
## 所以这只能证明，下版本Steam会检测某个软件的存在并弹窗提醒玩家卸载，但是他们用了“可能导致”这个词语，也就是并不代表他们一定会锁你账户（因为他们如果是严打作弊的态度，不会用“可能导致VAC”这种用词。），当然也有可能确实是检测一切解锁工具。具体情况也要等更新后再看。
## 不过就算被检测，根据这个更新内容，Steam也会先弹窗提醒你卸载，所以问题不大，如果看到弹窗就别用了。
## 不过如果真是检测一切解锁工具的话，那我也不知道怎么办了，也就只能停更仓库吧。
...
当然，厂商直接在Steam上设置过无法共享的游戏就不行(如GTA5, Elden Ring) ~~ 
...
GreenLuma_2024_x86.dll (版本GreenLuma_2024_1.5.8)
