...
#### Q: Why is the limit 142 games?

A: Since Greenluma is not open-source (at least I couldn’t find its source code), the maximum of 142 unlocks is a hardcoded limit set by the author. The reason might be that GreenLuma needs to spoof your game’s ID to bypass Steam’s validation, but the author only found 142 free games (accessible to everyone) that could be used for substitution.
