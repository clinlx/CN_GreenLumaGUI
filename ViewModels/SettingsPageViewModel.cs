...
namespace CN_GreenLumaGUI.ViewModels
{
    public class SettingsPageViewModel : ObservableObject
    {
        ...
        public RelayCommand RollbackSteamVersionCmd { get; set; }
        private static bool isRollbackWindowOpen = false;
        private void RollbackSteamVersion()
        {
            if (isRollbackWindowOpen) return;
            isRollbackWindowOpen = true;
            try
            {
                var win = new DateSelectionWindow();
                ...
                if (win.DataContext is DateSelectionViewModel vm && vm.IsConfirmed && vm.SelectedDate.HasValue)
                {
                    ManagerViewModel.Inform($"Try Rollback to {vm.SelectedDate.Value:yyyy-MM-dd}");
                    var res = OutAPI.RollbackSteamVersion(vm.SelectedDate.Value, out var time);
                    if (res)
                        ManagerViewModel.Inform($"Rollback Succeeded (SteamVersion: {time:yyyy-MM-dd})");
                    else
                        ManagerViewModel.Inform($"Rollback Failed (SteamVersion: {time:yyyy-MM-dd})");
                }
            }
            finally
            {
                isRollbackWindowOpen = false;
            }
        }
        //Bindings
