...
В: Це нормальний робочий стан програми:

Люди, які часто користуються сімейним доступом або повертали ігри, могли помітити: якщо гра вже завантажена, але ви втратили право на її запуск (наприклад, оформили повернення або хтось із сім'ї зараз у неї грає), Steam не показує кнопку «Грати» — замість неї з'являється кнопка «Купити».

Справжня функція цієї програми — примусово перетворити цю кнопку «Купити» назад на кнопку «Грати», дозволяючи вам все одно запустити гру.

Якщо у вас взагалі немає гри і на комп'ютері немає її завантажених файлів, то не бачити її в бібліотеці цілком нормально — немає й кнопки, яку можна було б відновити, а отже, функція програми просто не може проявитися.
...
Крім того, коли програма активна, функції Steam, пов'язані із завантаженням, наразі не працюють. Для ігор, якими ви володієте, перезапустіть Steam звичайним способом перед завантаженням або оновленням.
...
#### П: Для чого потрібна сторінка «Пул застосунків для підміни»?

В: Розблокування працює за принципом «тимчасово відмовитися від володіння однією грою, щоб отримати володіння іншою» — володіння застосунком із пулу тимчасово передається грі, яку ви хочете розблокувати. Тому скільки в пулі доступних застосунків — стільки ігор максимум можна розблокувати одночасно: **кількість доступних і є лімітом розблокування** (число у правому нижньому куті вікна).

На цій сторінці ви керуєте пулом:

* За умовчанням у пулі набір вбудованих безкоштовних застосунків (сірі, їх не можна видалити, але можна відключити). За умовчанням вони приховані — позначте «Показати вбудований пул», щоб побачити їх.
* Щоб підняти ліміт, введіть AppID гри, якою ви вже володієте (саме на своєму акаунті — ігри інших учасників сімейної групи не підходять), але поки не збираєтеся грати. Кожен доданий запис підвищує ліміт на 1.
* Кожен запис має позначку; знявши її, ви тимчасово відключаєте запис, а відключені записи не враховуються в кількості доступних.
* Спадний список фільтра переключає «Показати всі / Лише увімкнені / Лише відключені».
* Кнопка «Показати відповідності» відкриває вікно зі списком реальних пар «AppID із пулу → назва гри(ID гри)» у поточній конфігурації. Рядки з відступом — DLC.

Увага: поки гра з пулу реально використовується для підміни, ви не зможете нормально грати в неї під час сеансу з розблокуванням. Щоб знову грати, просто відключіть або видаліть цей запис — на ваше справжнє володіння грою це ніяк не впливає.

Усе вищесказане зумовлено тим, як влаштований сам GreenLuma. Усі дані пулу є суто локальними та жодним чином не взаємодіють із хмарою.


#### П: З багатьох ігор не розблокувалися лише кілька. Що робити?

В: Найімовірніше, один-два застосунки у вбудованому пулі AppID стали недійсними (наприклад, той безкоштовний застосунок вилучили з магазину або він більше не доступний усім). З якою грою опиниться в парі недійсний застосунок із пулу — та гра й не розблокується.

**Швидке рішення**: зніміть позначку з однієї-двох ігор на початку списку. Тоді весь порядок розблокування зсунеться, і гра, якій дістався недійсний застосунок, отримає робочий. Для початку можна грати так.

**Точніше рішення**: якщо ви знаєте, яка саме гра дає збій, натисніть «Показати відповідності» на сторінці «Пул застосунків для підміни», знайдіть AppID із пулу в парі з цією грою та вручну **відключіть** цей запис у списку. Після відключення він більше не бере участі в підміні, а всі наступні записи зсунуться автоматично.

(До речі, саме тому «розблокувалася лише частина DLC» іноді виглядає як обрив на певному рядку — причина та сама.)


#### П: Чому на кількість розблокувань є ліміт?

В: GreenLuma потрібно підмінити ID вашої гри, щоб обійти перевірку Steam, а безкоштовних ігор (доступних усім), які можна використовувати для заміни, обмежена кількість — тому певний ліміт існує. Число у правому нижньому куті програми і є поточним фактичним лімітом: воно дорівнює кількості доступних записів у «Пулі застосунків для підміни», де ви можете самостійно додавати, видаляти й відключати записи.
...
[2] Замініть DLL-файл: Помістіть ваш файл GreenLuma_2026_x**.dll у папку "override", не перейменовуючи його.
